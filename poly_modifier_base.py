...
NULL_VECTOR = numDataFn.object()
del numDataFn


def _register_new_obj_cb(list_):
    u"""lifted directly from pymel.genera._nodeAddCallback"""

    def callback(mobj, *args):
        # mobject handle are needed or pointers would goes null
        list_.append(
            om.MObjectHandle(mobj)
        )

    return callback


def _reset_pnts(tweak_plug):
    if not tweak_plug.isNull():
        for i in range(tweak_plug.numElements()):
            tweak = tweak_plug.elementByPhysicalIndex(i)
            tweak.setMObject(NULL_VECTOR)
...
class polyModifierCmd(ommpx.MPxCommand):
    ...
    def _redoModifyPoly(self):
        ...
        if not self._fHasHistory and not self._fHasRecordHistory:
            ...
            # pnts has to be reset here, and not after dag/dg modifier doit because it will
            # have cached the pnts by then.
            _reset_pnts(mesh_tweak_plug)
    ...
    def _create_duplicate_mesh_source(self, data, redo_data=None):
        ...
        # redo only works with pm.duplicate at the moment

        dag_fn = om.MFnDagNode(data.meshNodeShape)

        # data.upstreamNodeTransform = dag_fn.duplicate(False, False)
        # data.upstreamNodeTransform = pm.duplicate(dag_fn.fullPathName())[0].__apimobject__()

        # register callback, trap duplicated mobject and store to dup_mobj

        # start track
        new_obj_handles = []

        cid = om.MDGMessage.addNodeAddedCallback(_register_new_obj_cb(new_obj_handles))

        cmds.duplicate(dag_fn.fullPathName())

        # end track
        om.MMessage.removeCallback(cid)

        data.upstreamNodeTransform = new_obj_handles[0].object()
