...
def statusError(message):
    ...
    # raise Exception("statusError")  # called from exception handlers only, reraise exception
    assert False
    ...
    def __processUpstreamNode(self, data):
        ...
        if self.__fHasHistory:
            ...
            statusAssert(tempPlugArray.length() == 1,
                         "tempPlugArray.length() == 1 -- 0 or >1 connections on "
                    ...
                    for j in range(numChildren):
                        ...
                        if tweakChild.isConnected():
                            ...
                            if tweakChild.connectedTo(tempPlugArray, True, False):
                                ...
                                #
                                statusAssert(tempPlugArray.length() == 1,
                                             "tempPlugArray.length() == 1 -- 0 or >1 connections "
                                             "on tweakChild")
                ...
                for j in range(numChildren):
                    ...
                    if 0 < tweakSrcConnectionCountArray[i * numChildren + j]:
                        ...
                        while (k < tweakSrcConnectionCountArray[i * numChildren + j]):
                            self.__fDGModifier.connect(tweakChild,
                    ...
                    if 0 < tweakDstConnectionCountArray[i * numChildren + j]:
                        self.__fDGModifier.connect(tweakDstConnectionPlugArray[dstOffset],
                                                   tweakChild)
                        dstOffset += 1
...
#		(4) Define an instance of your specific polyModifierFty to perform the operation on the
# node
#		(5) Override the MPxNode::compute() method
#		(6) Inside compute():
#
